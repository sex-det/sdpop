...
int **countgenotypes(int npos, int nind, char *genotypes, int *sex) {
	char nucvec[4] = {0};
	int i,j,randbit;
...
void countfixed(int *nxy, int *nzw, int npolysites, int nFem, int nMal, int **polysite){
	int t;			
	int nZW=0,nXY=0;
	
	if(npolysites>0) {
		for (t=0; t<npolysites; t++){
			if (polysite[t][N12F]==nFem && (polysite[t][N11M]==nMal || polysite[t][N22M]==nMal)) {
				nZW++;
			}
			else if (polysite[t][N12M]==nMal && (polysite[t][N11F]==nFem || polysite[t][N22F]==nFem)) {
				nXY++;
			}			
		}
	}
...
int main(int argc, char *argv[]) {
	...
	int **counts,**permcounts,nperm;
	long double **siteprobs,**permsiteprobs;
	int npolysites,nf,nm,pos,ta,nA,nT,nG,nC,nN,fi,div;
	int ncontigs,totsites;
	int nnoncontigs;
	int ni,nXY,nZW,p_nXY,p_nZW;
...
//	feenableexcept(FE_ALL_EXCEPT & ~FE_INEXACT);
	...
	if (argc != 6) {
		fprintf(stdout,"Usage: %s infile outfile namelist1 namelist2 nperm\n",argv[0]);
		exit(1);
	}
	...
	while (ch != EOF) { //loop through the file
		...
		if ( line[0] == '>' ){
			if (firstcontig==0) {	//treat last read contig
				...
				//count genotypes
				counts=countgenotypes(npolysites,nfound,genotypes,foundsex);
				siteprobs=condsiteprobs(npolysites,counts);
				//count characteristic patterns
				countfixed(&nXY,&nZW,npolysites,nfem,nmal,counts);
...
//					sscanf(line,"\t%*c|%*c%[^\n]",tmpline);
			...
			if (div == 1) { //no polymorphism; discard
				n1++;
			}
			else if (div == 2) { //simple polymorphism; retain
				t++;
			}
